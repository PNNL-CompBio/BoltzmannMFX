...
\title{Model of Bacterial-Fungal Interactions on a Stationary Nutrient Medium}
...
\section{Model}
This document describes a fungal growth simulation in the presence of free swimming
bacteria. The simulation is implemented with the BMX framework. Individual fungal
hyphae are modeled as connected cylinders that 
...
The bacteria are modeled as free swimming particles that can interact with other
bacteria and fungal hyphae segments. The bacteria are also subject to a random force
that causes then to execute a random walk trajectory in the absence of any other bacteria
or fungal hyphae. Both bacteria and hyphae can absorb nutrients from the environment. The
fungi produce metabolites that are consumed by the bacteria and similarly, the bacteria
produce metabolites that are consumed by the fungi resulting in a partially
closed nutrient loop in which both bacteria and fungi support each other.

\section{Chemistry Model}
The chemistry for the combined bacteria-fungi simulation has been extended from
previous models to
include a total of five components, designated $A$, $B$, $C$, $D$, and $E$.
The chemistry of the first three components is similar to the model constructed for looking
at fungi alone and consists of a fairly simple set
of equations that describe chemical dynamics within individual fungal hyphae.
Inside the hyphae, only the constituents, $A$, $B$
and $C$ are considered. The cell volume is also coupled to these constituents. $A$ and $C$
...
\end{eqnarray*}
$A$, $B$ and $C$ are only capable of undergoing reactions inside the cell.  To distinguish
between concentrations of these components and absolute amounts of a component, we use
...
\[
B_{in} \stackrel{k_g,k_v}{\longrightarrow} V_{cell}
\]
...
The bacteria consume and modify components $C$, $D$, and $E$. In addition, one of the end
products of bacteria chemistry is component $A$, so the chemistries of both the fungi and
the bacteria are coupled to each other with the fungi producing $C$, which is consumed by
the bacteria, which then produces $A$. For the bacteria, components $C$ and $D$ are
permeable to the cell membrane, but $E$ is not. Inside the cell, the four constituents are
related by the reaction $C+D\longleftrightarrow A+E$. Combining this reaction with transport
across the bacterial membrane gives four additional equations
\begin{eqnarray*}
D_{out} &\stackrel{k_{4},k_{-4}}{\longleftrightarrow}& D_{in} \\
C_{in} + D_{in} &\stackrel{k_5,k_{-5}}{\longleftrightarrow}& A_{in} + E_{in} \\
A_{in} &\stackrel{k_6,k_{-6}}{\longleftrightarrow}& A_{out} \\
C_{out} &\stackrel{k_{7},k_{-7}}{\longleftrightarrow}& C_{in}
\end{eqnarray*}
Similar to the fungi, component $E$ is coupled to cell growth
\[
E_{in} \stackrel{k_{bg},k_{bv}}{\longrightarrow} V_{cell}
\]

The equations for $C$, $D$, $E$, and $A$ generate the following coupled rate equations
\begin{eqnarray*}
\frac{d [D_{out}]}{d t} &=&-k_4 [D_{out}]+k_{-4} [D_{in}] \\
\frac{d [D_{in}]}{d t} &=&k_4 [D_{out}]-k_{-4} [D_{in}] - k_5 [C_{in}][D_{in}]
 + k_{-5}[A_{in}][E_{in}]\\
\frac{d [E_{in}]}{d t} &=&k_5 [C_{in}][D_{in}] - k_{-5}[A_{in}][E_{in}] - k_{bg}[E_{in}]\\
\frac{d [A_{in}]}{d t} &=&k_5[C_{in}][D_{in}] - k_{-5}[A_{in}][E_{in}]-k_{-6} [A_{in}]
+ k_6 [A_{out}] \\
\frac{d [A_{out}]}{d t} &=&k_{-6} [A_{in}]-k_6 [A_{out}] \\
\frac{d [C_{out}]}{d t} &=&-k_7 [C_{out}]+k_{-7} [C_{in}] \\
\frac{d [C_{in}]}{d t} &=&k_7 [C_{out}]-k_{-7} [C_{in}] - k_5 [C_{in}][D_{in}]
 + k_{-5}[A_{in}][E_{in}]
\end{eqnarray*}
Note that although both $k_3$, $k_{-3}$ and $k_7$, $k_{-7}$ describe the transport
of $C$ across a cell membrane, it is a different membrane in each case and the coefficients
could potentially be different. A similar consideration applies to component $A$.
In analogy with hyphae growth, bacteria growth is governed by the equation
\[
\frac{dV_{cell}}{dt} = V_{cell}k_{bv}[E_{in}]
\]

In addition to transport in and out of the cell, coupled to diffusion in the external
growth medium, constituants can also be transferred between fungal hyphae that are directly
...
of the form
\[
f_{Xij} = M_X ([X_{in}]_i-[X_{in}]_j)
\]
where $f_{Xij}$ is the flux of component $X=A,C,E$ from segment $i$ into segment
$j$ and $M_X$ is the corresponding mass
...
We focus on component $C$ but similar comments apply to components $A$ and $D$. $B$ and $E$ are
impermeable to the cell membranes so their fluxes in and out of the cell are zero. We define
coefficients $k'_3$ and $k'_{-3}$ as mass-transfer coefficients and the fluxes of material into the
cell, $f_{in}$, and out, $f_{out}$, can be written as
\begin{eqnarray*}
...
rate equations governing the fungi become
\begin{eqnarray*}
...
\end{eqnarray*}
Similarly, the equations for the bacteria are
\begin{eqnarray*}
\frac{d [A_{out}]}{d t} &=&-k'_6\frac{A_{cell}}{V_{grid}} [A_{out}]
+k'_{-6}\frac{A_{cell}}{V_{grid}} [A_{in}] \\
\frac{d [C_{out}]}{d t} &=&-k'_7\frac{A_{cell}}{V_{grid}} [C_{out}]
+k'_{-7}\frac{A_{cell}}{V_{grid}} [C_{in}] \\
\frac{d [D_{out}]}{d t} &=&-k'_4\frac{A_{cell}}{V_{grid}} [D_{out}]
+k'_{-4}\frac{A_{cell}}{V_{grid}} [D_{in}] \\
\frac{d [A_{in}]}{d t} &=& -k'_{-6}\frac{A_{cell}}{V_{cell}} [A_{in}]
+ k'_6\frac{A_{cell}}{V_{cell}} [A_{out}] +
k_5[C_{in}][D_{in}] - k_{-5}[A_{in}][E_{in}] \\
\frac{d [C_{in}]}{d t} &=&k'_7\frac{A_{cell}}{V_{cell}} [C_{out}]
-k'_{-7}\frac{A_{cell}}{V_{cell}} [C_{in}] - k_5 [C_{in}][D_{in}]
 + k_{-5}[A_{in}][E_{in}] \\
\frac{d [D_{in}]}{d t} &=&k'_4\frac{A_{cell}}{V_{cell}} [D_{out}]
-k'_{-4}\frac{A_{cell}}{V_{cell}} [D_{in}] - k_5 [C_{in}][D_{in}]
 + k_{-5}[A_{in}][E_{in}] \\
\frac{d [E_{in}]}{d t} &=& k_5 [C_{in}][D_{in}]
 - k_{-5}[A_{in}][E_{in}]
\end{eqnarray*}
From here on, we drop the primes on $k'_1$, $k'_{-1}$, etc.

Numerically, the rate equations are currently being handled using a scheme that splits the
transport across the cell membrane, the reactions inside the cell, and the transport between
hyphae into separate steps. These consist of the following
\begin{list}{$\bullet$}{}
...
using the updated internal concentrations from the previous step. This reaction does
not include the conversion of components $B$ and $E$ into cell volume
\item The reactions converting $B$ and $E$ into cell volume are evaluated over a time
increment $\dt$
...
$\dt/2$
\item After the transport across the membrane and internal reactions are completed,
the exchange between hyphae is done over a time increment $\dt$.
\end{list}
...
The discussion below focuses on the reactions associated with the hyphae, but the bacterial
reactions are completely analogous.  The total amount of
material that flows into the cell in the interval $\dt/2$ is
\begin{eqnarray*}
...
After completing the internal reactions, the reaction leading to cell growth is evaluated.
The current value of $[B_{in}]$ is modified using
\[
[B'_{in}] = [B_{in}]-k_g\dt[B_{in}]
\]
and the volume is updated with
\[
V'_{cell} = V_{cell}+k_v V_{cell}\dt[B_{in}]
\]
The equations for modifying the bacteria volume based in $[E_{in}]$ are
completely analogous.

Finally, the transfer of material between hyphae needs to be evaluated.
The actual amount of material transferred over a short time interval $\Delta t$ is the
flux multiplied by the area of the hyphae, $A_{ij}$, and the time interval
\[
\Delta X_i = -\Delta X_j = -\dt A_{ij} M_{X}([X_{in}]_i-[X_{in}]_j)
\]
The area of the septum is taken as the average of the area of the end caps of the two joined
hyphae.

It is again necessary to maintain positive
concentrations within the hyphae. Theoretically, the transfer of material between hyphae should
be accomplished solving a set of simultaneous equations for transfer between all hyphae in th
...
setting up these kind of equations in a distributed environment, the overhead of doing a
...
separate portions $X/M$. This portion is then the maximum amount that can be exchanged with
...
Our model for interacting cylinders is based on finding the minimum distance between two
