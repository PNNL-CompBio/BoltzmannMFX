...
produce metabolites that are consumed by the fungi.

\section{Chemistry Model}
The chemistry for the combined bacteria-fungi simulation has been extended to
...
of equations that describe dynamics within individual fungal hyphae.
...
\frac{d [C_{in}]}{d t} &=&k_7 [C_{out}]-k_{-7} [C_{in}] - k_5 [C_{in}][D_{in}]
 + k_{-5}[A_{in}][E_{in}]
\end{eqnarray*}
Note that although both $k_3$, $k_{-3}$ and $k_7$, $k_{-7}$ both describe the transport
...
\[
\frac{dV_{cell}}{dt} = V_{cell}k_bv[E_{in}]
\]
...
\]
where $f_{Xij}$ is the flux of component $X=A,C,E$ from segment i into segment j and $M_X$ is
...
\frac{d [A_{in}]}{d t} &=&k_5[C_{in}][D_{in}] - k_{-5}[A_{in}][E_{in}]
-k'_{-6}\frac{A_{cell}}{V_{cell}} [A_{in}] + k'_6\frac{A_{cell}}{V_{cell}} [A_{out}] \\
